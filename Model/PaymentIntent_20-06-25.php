...
class PaymentIntent extends \Magento\Framework\Model\AbstractModel
{
    ...
    public function getParamsFrom($quote, $order = null, $paymentMethodId = null)
    {
       ...
       // echo "S: ".$seller_id;exit;
        ...
        $dynamicCurrency = (isset($selletAttributeCollection['seller_currency_symbol'])) ? $selletAttributeCollection['seller_currency_symbol'] : $this->getConfigData('currency_code');
        if($dynamicCurrency=='OMR' || $dynamicCurrency=='KWD' ||$dynamicCurrency=='BHD' || $dynamicCurrency=='JOD'){
            $cents = 1000;
        } else { //default
            $cents = 100;
        }
        $params['amount'] = round($amount * $cents);
        $params['currency'] = strtolower($dynamicCurrency); //strtolower($currency)
