...
class PaymentIntent extends \Magento\Framework\Model\AbstractModel
{
    ...
    public function getParamsFrom($quote, $order = null, $paymentMethodId = null)
    {
        ...
        if ($seller_id > 0) {
            ...
            $currency = (isset($selletAttributeCollection['seller_currency_symbol'])) ? $selletAttributeCollection['seller_currency_symbol'] : $this->getConfigData('currency_code');
            if($currency=='OMR' || $currency=='KWD' ||$currency=='BHD' || $currency=='JOD'){
                $cents = 1000;
                $params['amount'] = round($amount * 1000);
            } else { //default
                $cents = 100;
